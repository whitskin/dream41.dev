<template>
  <div class="relative overflow-x-hidden selection:bg-indigo-500/30 selection:text-indigo-500">
    ...
    <div class="fixed inset-0 z-0 pointer-events-none overflow-hidden">
       <div class="absolute inset-0 opacity-[0.03] dark:opacity-[0.07] bg-[url('https://grainy-gradients.vercel.app/noise.svg')] mix-blend-overlay"></div>
       <div class="hidden dark:block absolute top-[-10%] left-[20%] w-[600px] h-[600px] bg-indigo-600/20 rounded-full blur-[120px] animate-blob"></div>
       <div class="hidden dark:block absolute bottom-[-10%] right-[-10%] w-[500px] h-[500px] bg-purple-600/20 rounded-full blur-[120px] animate-blob" style="animation-delay: 2s"></div>
       <div class="block dark:hidden absolute top-[-20%] right-[-10%] w-[800px] h-[800px] bg-blue-100/80 rounded-full blur-[100px]"></div>
    </div>
    ...
    <header class="fixed top-6 inset-x-0 z-50 flex justify-center px-6" v-motion :initial="{ y: -100, opacity: 0 }" :enter="{ y: 0, opacity: 1, transition: { stiffness: 100, damping: 15 } }">
      <nav class="bg-white/80 dark:bg-black/60 backdrop-blur-2xl border border-zinc-200 dark:border-white/10 rounded-full py-2.5 px-6 shadow-xl shadow-black/5 dark:shadow-black/20 flex items-center justify-between w-full max-w-5xl transition-all hover:border-indigo-500/20">
        ...
        <div class="flex items-center gap-2">
           <button @click="toggleSnow" class="group p-2.5 rounded-full hover:bg-zinc-100 dark:hover:bg-white/10 transition-colors border border-transparent dark:border-white/5 relative" :class="isSnowing ? 'bg-indigo-50 text-indigo-600 dark:bg-indigo-500/20 dark:text-indigo-400' : 'text-zinc-500 dark:text-zinc-400'">
           ...
           <button @click="toggleTheme" class="group p-2.5 rounded-full text-zinc-500 hover:bg-zinc-100 dark:text-zinc-400 dark:hover:bg-white/10 transition-colors border border-transparent dark:border-white/5">
             <ClientOnly>
    ...
    <main class="relative z-10 pt-44 pb-20 px-6 max-w-7xl mx-auto space-y-40">
      ...
      <section class="text-center space-y-8 relative">
        ...
        <h1 v-motion :initial="{ opacity: 0, y: 50 }" :enter="{ opacity: 1, y: 0, transition: { delay: 200, duration: 800, type: 'spring' } }" class="text-6xl sm:text-7xl md:text-9xl font-black text-zinc-900 dark:text-white tracking-tighter leading-[0.9]">
          Building the <br />
        ...
        <div v-if="discordData" v-motion :initial="{ opacity: 0, scale: 0.9 }" :enter="{ opacity: 1, scale: 1, transition: { delay: 500 } }" class="flex justify-center mt-6">
           <div class="flex items-center gap-4 p-2 pr-6 rounded-full bg-white dark:bg-zinc-900/80 border border-zinc-200 dark:border-white/10 shadow-lg backdrop-blur-md">
              ...
              <div class="text-left">
                 ...
                 <div class="text-sm font-bold text-zinc-900 dark:text-white truncate max-w-[200px]">
                    {{ discordData.listening_to_spotify ? discordData.spotify.song : discordStatusText }}
                 </div>
              </div>
              <div v-if="discordData.listening_to_spotify" class="flex gap-1 items-end h-3">
      ...
      <section id="skills" v-motion-fade-visible class="w-full border-y border-zinc-200 dark:border-white/10 bg-white/50 dark:bg-black/30 py-16 overflow-hidden backdrop-blur-sm">
        <div class="relative flex overflow-x-hidden group [mask-image:linear-gradient(to_right,transparent,black_20%,black_80%,transparent)]">
            <div class="flex gap-16 animate-marquee whitespace-nowrap hover:[animation-play-state:paused]">
              <div class="flex gap-16 items-center shrink-0">
                  <div v-for="skill in skills" :key="skill.name" class="flex items-center gap-3 opacity-50 hover:opacity-100 transition-all duration-300 grayscale hover:grayscale-0 cursor-default hover:scale-110">
                    <img :src="skill.icon" :alt="skill.name" class="w-10 h-10 object-contain drop-shadow-lg" />
      ...
      <section id="contact" class="pb-20">
         <div v-motion :initial="{ opacity: 0, scale: 0.8, y: 100 }" :visibleOnce="{ opacity: 1, scale: 1, y: 0, transition: { type: 'spring', stiffness: 40 } }" class="relative rounded-[3rem] bg-zinc-900 dark:bg-[#050505] overflow-hidden px-8 py-32 text-center border border-zinc-800 dark:border-white/10 shadow-2xl dark:shadow-black/50 group">
            <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-indigo-500/20 blur-[120px] rounded-full pointer-events-none group-hover:bg-indigo-500/30 transition-all duration-1000"></div>
            <div class="relative z-10 max-w-2xl mx-auto space-y-8">
               ...
               <div class="flex flex-wrap justify-center gap-4 pt-4">
                  <a href="mailto:contact@dream41.dev" class="flex items-center gap-2 px-8 py-4 rounded-full bg-white text-zinc-900 font-bold text-lg hover:scale-105 transition-transform">
                     <EnvelopeIcon class="w-5 h-5" /> Email Me
                  </a>
                  <a href="https://discord.com/users/1147956655384956978" target="_blank" class="flex items-center gap-2 px-8 py-4 rounded-full bg-white/10 backdrop-blur-md text-white font-bold text-lg border border-white/10 hover:bg-white/20 transition-all">
    ...
    <footer class="border-t border-zinc-200 dark:border-white/10 py-12 bg-white dark:bg-black">
       <div class="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <p class="text-sm text-zinc-400">© 2025. Built with Nuxt 4 & Motion.</p>
       </div>
    </footer>
  </div>
</template>
